...
function updateBackground()
{
    ...
    jQuery('body').css({"background":"url("+BackgroundImage+") "+BackgroundRepeat+" fixed "});
	jQuery('body').css({"background-position":"0px 39px"});
	jQuery('body').css({"background-color":BackgroundColor});
...
function updateWord(text, status, minWords, wordsAllow) {
	currentWordCount = countWords(text);

	if (currentWordCount<minWords) {
		$(status).html('' + (minWords - countWords(text)) + ' words left to finish' );
	} else
	{
		$(status).html('' + (wordsAllow - countWords(text)) + ' words available' );
	}
}
