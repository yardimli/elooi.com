...
class Readability2 {
    ...
    // 当判定失败时显示的内容
    const MESSAGE_CAN_NOT_GET = "Readability2 was unable to parse this page for content.";
    ...
    private function getTopBox() {
        ...
        for ($i = 0, $len = sizeof($this->parentNodes); $i < $len; $i++) {
            ...
            $contentScore    = intval($parentNode->getAttribute(Readability2::ATTR_CONTENT_SCORE));
            $orgContentScore = intval($topBox ? $topBox->getAttribute(Readability2::ATTR_CONTENT_SCORE) : 0);
    ...
    public function getContent() {
        ...
        if($ContentBox === null)
            throw new RuntimeException(Readability2::MESSAGE_CAN_NOT_GET);
        ...
        $content = mb_convert_encoding($Target->saveHTML(), Readability2::DOM_DEFAULT_CHARSET, "HTML-ENTITIES");

        // 多个数据，以数组的形式返回
        return Array(
            'lead_image_url' => $this->getLeadImageUrl($Target),
            'word_count' => mb_strlen(strip_tags($content), Readability2::DOM_DEFAULT_CHARSET),
