...
<div style="margin-left:0px; z-index:0; width:650px; background:#ffffff; " id="grid_centercol">
<?php
if ($UserID>0)
{
...
<div id="elooi_<?php echo $arraycounter; ?>" class="elooi_row" onMouseOver="javascript:selectRow('<?php echo $arraycounter; ?>');" onMouseOut="javascript:deselectRow('<?php echo $arraycounter; ?>')" onClick="javascript:RowClick('<?php echo $arraycounter; ?>');" <?php   if ($arraycounter % 2 == 0) { echo "style=\"background:#F1F1F5;\""; } ?> >
	<div id="dogear_<?php echo $arraycounter; ?>" class="tweet-dogear <?php
		...
		if ($_SESSION['Elooi_UserID']!="") { //if user has logged in start check
			...
			if ($UseMyInbox == 0) {
				$favCmd = "SELECT ID FROM eloois WHERE Deleted=0 AND EchoElooiID=".mysql_result($mysqlresult,$i,"ID")." AND EchoUserID=".$_SESSION['Elooi_UserID'];
				$favRes = mysql_query($favCmd);
				if (mysql_num_rows($favRes)==1) { $unecho = 1; }
			} else
			{
	...
	<div onClick="javascript:event.stopPropagation(); window.location='/d/<?php echo mysql_result($mysqlresult,$i,"ResponseToElooiID"); ?>'" id="reply_<?php echo $arraycounter; ?>" class="eloo-reply-icon"></div>
	<?php }	?>
	...
	<div style="margin-left:60px; min-height:52px;">
		...
		<?php
			...
			$tag_icon="ui-icon ui-icon-tag tag-cloud-icon";
			//dont show echo, echo wont have its own tags on version 2.00
			if ((mysql_result($mysqlresult,$i,"EchoElooiID")!=0) && (1==1)) { 
				...
				while ($i_tags<$num_rows_tags)
				{
					...
					?><span class="tag_cloud" onClick="javascript:event.stopPropagation(); window.location='/etiket/<?php echo $atag; ?>'"><span class="<?php 
						if (substr($atag,0,1)=="@") {echo "ui-icon ui-icon-person tag-cloud-icon"; } //else { echo $tag_icon; } ?>"></span><?php
					echo $atag; 
					?></span><?php
					$i_tags++;
				}
			...
			} else
			{
				...
				while ($i_tags<$num_rows_tags)
				{
					$atag = mysql_result($mysqlresult_tags,$i_tags,"alltags.tag");
						//<span class="<?php 
						//if (substr($atag,0,1)=="@") {echo "ui-icon ui-icon-person tag-cloud-icon"; } else { echo $tag_icon; } ? >"></span>

						if (substr($atag,0,1)=="@") { 
							$xsqlCommand_mention_picture = "SELECT * FROM users WHERE username =". ATQ(substr($atag,1)) ."";
							$mysqlresult_mention_picture = mysql_query($xsqlCommand_mention_picture);
							$num_rows_mention_picture = mysql_num_rows($mysqlresult_mention_picture);
							if ($num_rows_mention_picture==1) {
							?>
							<span class="tag_cloud" onClick="javascript:event.stopPropagation(); window.location='/d/<?php echo mysql_result($mysqlresult,$i,"ResponseToElooiID"); ?>'"><img src="/slir/w52-h52-c1.1/<?php echo mysql_result($mysqlresult_mention_picture,0,"picture"); ?>"></span>
							<?php
							//echo "-".$atag;
							}
						} else { ?>
						<span class="tag_cloud" onClick="javascript:event.stopPropagation(); window.location='/etiket/<?php echo $atag; ?>'"><?php
						echo $atag; } ?></span><?php
					$i_tags++;
				}
			}
			?><?php 
				//if (mysql_result($mysqlresult,$i,"MusicCredit")!="") {? ><span class="tag_cloud" style=" cursor:default;" onclick="javascript:event.stopPropagation();" ><span class="ui-icon ui-icon-volume-on tag-cloud-icon"></span><? php echo mysql_result($mysqlresult,$i,"MusicCredit"); ? ></span><? php } 
			?><?php 
			if (mysql_result($mysqlresult,$i,"Picture")!="") {?><span class="tag_cloud" onClick="javascript:show_photo('/audio-picture/<?php echo mysql_result($mysqlresult,$i,"Picture"); ?>',event);  return false;"><?php echo "<img src=\"/slir/w48-c1.1/audio-picture/".mysql_result($mysqlresult,$i,"Picture")."\" style=\"height:48px; width:48px; margin-left:4px;\" ></span>";
			} 

			if (mysql_result($mysqlresult,$i,"LinkURL")!="")
			{
				//<span class=\"ui-icon ui-icon-extlink tag-cloud-icon\"></span>
				echo "<span class=\"tag_cloud\"><a  onClick=\"javascript:event.stopPropagation();\" target=\"_blank\" rel=\"nofollow\" title=\"". mysql_result($mysqlresult,$i,"LinkURL") ."\" href=\"". mysql_result($mysqlresult,$i,"ShortURL") . "\">";
				echo "<img src=\"/slir/w48-c1.1/document-url-icon.png\" style=\"height:48px; width:48px; margin-left:4px\">"; 
		...
		<div style="display:block; margin-top:8px; margin-left:4px; margin-bottom:0px; height:8px; font-size:11px; ">
			...
			<span id="action_barn_<?php echo $arraycounter; ?>" style="display:none">
				<span class="elooi-actions">
					...
					<a class="add-picture-action" onclick="retag_elooi_by_ID('<?php echo $ElooiID; ?>',event); return false;" href="#"><span><i></i><b><?php echo $me_add_picture; ?></b></span></a>
					
					<a class="add-url-action" onclick="retag_elooi_by_ID('<?php echo $ElooiID; ?>',event); return false;" href="#"><span><i></i><b><?php echo $me_add_link; ?></b></span></a>
